...
// GET /api/admin/content/:blogId/comments - Get all comments for moderation
export async function getCommentsForModeration(req, res, next) {
  try {
    const { blogId } = req.params;

    const blog = await Content.findById(blogId)
      .populate("comments.moderatedBy", "name email")
      .lean();
    ...
    const pendingComments = (blog.comments || [])
      .filter((comment) => comment.status === "pending")
      .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));

    // Get recent moderated comments
    const moderatedComments = (blog.comments || [])
      .filter((comment) => comment.status !== "pending")
      .sort(
        (a, b) =>
    ...
    res.json({
      ...
      moderatedComments,
      stats: {
        total: blog.comments?.length || 0,
        pending: pendingComments.length,
        approved:
          blog.comments?.filter((c) => c.status === "approved").length || 0,
        rejected:
          blog.comments?.filter((c) => c.status === "rejected").length || 0,
      },
    });
  } catch (err) {
    next(err);
